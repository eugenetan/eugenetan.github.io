...
function mp3Link(book, content) {
    if (!book || !book.mp3 || !content)
        return "";
    return "<a href='mp3/" + encodeURIComponent(book.mp3).replace(" ", "+") + "'>" + content + "</a> ";
}
...
function narratorLink(book) {
    if (!book || !book.narrated_by)
        return "";
    return "<a href='https://www.audible.com/search?searchNarrator=" + encodeURIComponent(book.narrated_by).replace(" ", "+") + "'>" + asString(book.narrated_by) + "</a> ";
}

function bookImage(book, addLink) {
    var image = (book && book.image && book.image.length>0) ? ("<img src='thumb/" + encodeURIComponent(book.image).replace(" ", "+") + "' width='200' height='200'>")
        : "<img src='assets/no-cover.png' width='200' height='200'>";
    if (addLink)
        return mp3Link(book, image);
    return image;
}
